 ...
 *
 **/
public class Dondtwel_old implements TicketDataGenerator {
    ...
    private final static String VERSION = "c";
    ...
    private final static int TOP_PRIZE = 250000;
    ...
    private final static int HIGH_PRIZE = 0;
    private final static int MEDIUM_PRIZE = 1;
    private final static int LOW_PRIZE = 2;
    ...
    public String getTicketData(GameOutcome gameOutcome) throws com.camelotinteractive.game.instant.TicketDataException {
        ...
        for( Iterator it = prizes.iterator() ; it.hasNext();) {
            //xml.append(this.formatDecimal((Integer) it.next()));
            xml.append(it.next());
            if (it.hasNext()) {
                xml.append(",");
            }
        ...
        for (int i = 0; i < PHASE_ONE_ROWS; i++) {
            ...
            if (sameColourCount > 5) {                
                throw new TicketDataException("Too many same colour prizes in a row");
            }
            xml.append("<turn n=\"");
            xml.append(i);            
            xml.append("\" p=\"");
    ...
    private void generateGame(int tier, boolean isWinner) throws TicketDataException {
        ...
        Boolean hasTopPrize = false;
        Boolean hasHighPrize = false;
        Boolean hasMediumPrize = false;
        Boolean hasLowPrize = false;
        ...
        /*System.out.println(Arrays.deepToString(this.phaseTwoBucketOne.toArray()));
        System.out.println(Arrays.deepToString(this.phaseTwoBucketTwo.toArray()));
        System.out.println(Arrays.deepToString(this.phaseTwoBucketThree.toArray()));*/
        
        if (this.gameOneRows.size() < PHASE_ONE_ROWS || this.gameTwoRows.size() < PHASE_TWO_ROWS) {
            //System.out.println(Arrays.deepToString(this.gameThreeRows.toArray()));
            throw new TicketDataException("Ticket has not been completely filled");
        }
        if (this.gameOneRows.size() > PHASE_ONE_ROWS || this.gameTwoRows.size() > PHASE_TWO_ROWS) {
            //System.out.println(this.gameOneRows.size() + " " + this.gameThreeRows.size() + " " + this.gameTwoRows.size());
     ...
     * @return int
     */
    public void placeWinnerInPhaseOne(int prize) throws TicketDataException {
     ...
     * @return int
     */
    public void placeInPhaseOne(int prize) throws TicketDataException {
        int pos = 1; //this.randomBetween(1, 4);
        boolean placed = false;
        int count = 0;
        Integer[] row = new Integer[2];
        row[0] = prize;
     ...
     * @return int
     */
    public void placeInPhaseTwo(int prize) throws TicketDataException {
     ...
     * @return int
     */
    public void placeWinnerInPhaseTwo(int prize, int number) throws TicketDataException {
        Integer[] row = null;
        if (number > 1) {
            row = new Integer[2];
            row[0] = prize;        
